...
public class Flashlight1 : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        ...
        {
            // 월드 좌표 기준 카메라의 위치를 손전등의 로컬 좌표로 변환
            flashlightLight.transform.position = cameraTransform.TransformPoint(Vector3.zero);
            flashlightLight.transform.rotation = cameraTransform.rotation;

            Debug.Log("손전등 위치 동기화 완료: " + flashlightLight.transform.position);
